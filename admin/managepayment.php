...
if (isset($_REQUEST[send])) {
    $get = mysql_query("select * from paymethod where payname like '$_REQUEST[payname]' and url like '$_REQUEST[url]' ");
    $g = mysql_fetch_array($get);

    if ($g[0] == "") {
        $ins = mysql_query("insert into paymethod values(0,'$_REQUEST[payname]','$_REQUEST[url]',0)");
    } else {
        $er = 1;
    }
}
